    ...
    <div class="dash-grid">
      ...
      <div class="dash-card">
        ...
        <div class="vr-row">
          ...
          ${v.fotos&&v.fotos.filter(Boolean).length?`<img data-firmar="${v.fotos[0]}" style="width:46px;height:34px;object-fit:cover;border-radius:5px;border:1px solid var(--border);margin-left:8px;flex-shrink:0;cursor:pointer" onclick="openLightbox('${v.fotos[0]}')">`:''}
        </div>`).join('')}
      </div>
      <div class="dash-card">
...
async function _urlToBase64(url){
  const firmada=await resolverUrlFirmada(url);
  return new Promise((resolve,reject)=>{
