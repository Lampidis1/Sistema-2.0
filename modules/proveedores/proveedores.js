  ...
  el.innerHTML=`
    ...
    <div class="dash-stat-big">
      ...
      <div class="stat-big-card">
        ...
        <div class="stat-big-label">Licitaciones vigentes</div>
      </div>
    </div>
    <div class="dash-grid">
      <div class="dash-card">
      ...
      <div class="dash-card">
        ...
        hotels.filter(p=>(DB.hoteles[p._id]||{}).total>0).sort((a,b)=>{
          ...
          return Math.max(0,hb.total-(hb.contratos||[]).reduce((s,c)=>s+ctHabs(c),0))-Math.max(0,ha.total-(ha.contratos||[]).reduce((s,c)=>s+ctHabs(c),0));
        }).map(p=>{
  ...
  el.innerHTML=`<div style="padding:4px 0 0">
    ...
    ${acs.map(ac=>{
      ...
      return `<div class="ac-card ${cls==='warn'||cls==='danger'?'vence-pronto':cls==='vencido'?'vencido':''}">
        ...
        ${(ac.foto1_url||ac.foto2_url||ac.foto3_url)?`<div style="display:flex;gap:6px;margin-top:6px">${['foto1_url','foto2_url','foto3_url'].filter(k=>ac[k]).map(k=>`<a href="${ac[k]}" target="_blank"><img src="${ac[k]}" style="width:54px;height:42px;object-fit:cover;border-radius:5px"></a>`).join('')}</div>`:''}
        ...
        <div class="lic-mini-actions">
          ...
          ${ac.pdf_url?`<a class="mini-btn" style="width:auto;padding:4px 10px" href="${ac.pdf_url}" target="_blank">📄 PDF</a>`:''}
...
async function onSesionIniciada(){
  ...
  ocultarLogin();
  // mostrar pestaña Usuarios solo a admins
  const navU=document.getElementById('navUsuarios'); if(navU) navU.style.display=ES_ADMIN_ACTUAL?'inline-flex':'none';
  // cargar/crear perfil (nombre y apellido)
  await cargarPerfil(email);
  // refrescar badge de solicitudes pendientes + RESPALDO: si el servidor deja listar, es admin real
  try{
    const {data,error}=await SUPA.client.rpc('listar_solicitudes');
    if(!error && Array.isArray(data)){
      // el servidor confirmó rol admin → asegurar visibilidad de la pestaña aunque el JWT local estuviera desactualizado
      if(!ES_ADMIN_ACTUAL){ ES_ADMIN_ACTUAL=true; document.body.classList.add('is-admin'); document.body.classList.remove('is-user'); if(navU) navU.style.display='inline-flex'; }
      actualizarBadgeUsuarios(data.filter(s=>s.estado==='pendiente').length);
    }
  }catch(e){}
...
// ═══════════════════════════════════════════════════════════════════════════
// VISTA AGENDA TELEFÓNICA + NUEVO PROVEEDOR (v2.1)
// ═══════════════════════════════════════════════════════════════════════════
...
function editarLicitacion(pid, aid){
  ...
  // previews archivos
  document.getElementById('licEditPdfLink').innerHTML=ac.pdf_url?`<a href="${ac.pdf_url}" target="_blank">📄 Ver PDF actual</a>`:'<span style="color:var(--text-muted)">Sin PDF</span>';
  ['1','2','3'].forEach(n=>{
    const u=ac['foto'+n+'_url'];
    document.getElementById('licFotoPrev'+n).innerHTML=u?`<img src="${u}" style="width:100%;height:70px;object-fit:cover;border-radius:6px">`:'';
  });
...
function minutaHTML(v){
  ...
  return `<div style="font-family:'Barlow',sans-serif;color:#1f2a2c">
    ...
    ${(v.fotos&&v.fotos.filter(Boolean).length)?`<div style="margin-top:14px"><div style="font-weight:800;color:#006973;font-size:.8rem;text-transform:uppercase;border-bottom:1px solid #ddd;padding-bottom:3px">Fotografías de la visita</div>
      <div style="display:flex;gap:8px;margin-top:6px">${v.fotos.filter(Boolean).slice(0,3).map(u=>`<img src="${u}" style="width:31%;aspect-ratio:1.4;object-fit:cover;border-radius:6px;border:1px solid #ddd"/>`).join('')}</div>
    </div>`:''}
  ...
  try{
    ...
    cont.innerHTML=data.map(v=>`<div class="vis-item">
      ...
      <div style="display:flex;gap:5px">
        ...
        ${v.minuta_pdf_url?`<a href="${v.minuta_pdf_url}" target="_blank" class="mini-btn" title="PDF guardado">☁️</a>`:''}
...
async function renderUsuarios(){
  ...
  if(!cont) return;
  if(!ES_ADMIN_ACTUAL){ cont.innerHTML='<div class="kb-empty">Solo el administrador puede gestionar usuarios.</div>'; return; }
  cont.innerHTML='<div class="kb-empty">Cargando…</div>';
  try{
    const {data,error}=await SUPA.client.rpc('listar_solicitudes');
    if(error) throw error;
...
async function aprobarUsuario(uid){
  ...
  try{
    const {data,error}=await SUPA.client.rpc('aprobar_usuario_v2',{p_uid:uid,p_rol:rol,p_accesos:accesos});
    if(error) throw error;
...
async function rechazarUsuario(uid){
  ...
  try{
    const {error}=await SUPA.client.rpc('rechazar_usuario',{p_uid:uid});
    if(error) throw error;
...
// ═══════════════════════════════════════════════════════════════════════════
// CATÁLOGO DE PROGRAMAS / INICIATIVAS v6.0
...
// ═══════════════════════════════════════════════════════════════════════════
// DASHBOARD DE COMPROMISOS (cuenta regresiva + seguimiento/llamadas) v7.0
// ═══════════════════════════════════════════════════════════════════════════
...
// Sincronizar visibilidad de "Usuarios" en el menú móvil
(function(){
  const obs=()=>{ const navU=document.getElementById('navUsuarios'); const mU=document.getElementById('mobileNavUsuarios');
    if(navU&&mU) mU.style.display = (navU.style.display==='none'?'none':'block'); };
  setInterval(obs, 1500);
})();


// ═══ EXPORTAR FICHA EN PDF (jsPDF, estética AMSA) v12 ═══
...
const LOGOS_FAENA={Centinela:"data:image/png;base64,",Antucoya:"data:image/png;base64,","Zaldívar":"data:image/png;base64,",Zaldivar:"data:image/png;base64,"};

// Convierte una URL de imagen (Storage) a base64 dataURL para incrustar en PDF
function _urlToBase64(url){
  return new Promise((resolve,reject)=>{
...
async function exportarFichaPDF(id){
  ...
  try{
    ...
    doc.setFont('helvetica','normal'); doc.setFontSize(7);
    doc.text('Región de Antofagasta · '+new Date().toLocaleDateString('es-CL'),W-M,19,{align:'right'});
    y=44;

    // ── Nombre y localidad ──
    doc.setTextColor(...DARK); doc.setFont('helvetica','bold'); doc.setFontSize(19);
    const nameLines=doc.splitTextToSize(name,maxW); doc.text(nameLines,M,y); y+=nameLines.length*7.5;
    if(name.toLowerCase()!==(p.razon_social||'').toLowerCase()&&p.razon_social){
      doc.setFont('helvetica','normal'); doc.setFontSize(9.5); doc.setTextColor(...GRAY);
      doc.text(p.razon_social,M,y); y+=5.5;
    }
    doc.setFontSize(9); doc.setTextColor(...TEAL2); doc.setFont('helvetica','bold');
    doc.text((p.localidad||'')+(p.rango_trabajos?'   ·   '+p.rango_trabajos:''),M,y); y+=3;
    doc.setDrawColor(...GOLD); doc.setLineWidth(.8); doc.line(M,y,M+30,y); y+=8;

    // ── Función helper de sección ──
    function seccion(titulo){
      if(y>262){doc.addPage();y=20;}
      doc.setFillColor(...LT); doc.rect(M,y-4.5,maxW,7,'F');
      doc.setFont('helvetica','bold'); doc.setFontSize(9); doc.setTextColor(...TEAL);
      doc.text(titulo.toUpperCase(),M+2,y); doc.setTextColor(...DARK); y+=8;
    }
    function campo(label,val){
      ...
      const txt=doc.splitTextToSize(String(val),maxW-lw-3); doc.text(txt,M+lw+3,y); y+=txt.length*5+1.5;
    }

    // ── Rubros ──
    seccion('Rubros y giros');
    campo('Rubros: ',(p.rubrosNorm||[]).join(' · '));
    campo('Giros SII: ',(p.giros||[]).join(' · '));
    y+=2;

    // ── Contacto ──
    seccion('Contacto');
    if(cp){ campo('Nombre: ',cp.nombre); campo('Cargo: ',cp.cargo); campo('Teléfono: ',cp.fono||p.fono); campo('Correo: ',cp.correo||p.correo); }
    else { campo('Teléfono: ',p.fono); campo('Correo: ',p.correo); }
    campo('Dirección: ',p.direccion);
    // contactos secundarios
    if(cs.length>1){ y+=1; doc.setFont('helvetica','bold');doc.setFontSize(8);doc.setTextColor(...GRAY);doc.text('Otros contactos:',M,y);y+=4.5;
      cs.slice(1).forEach(c=>{ campo('  • ',c.nombre+(c.cargo?' ('+c.cargo+')':'')+(c.fono?' · '+c.fono:'')+(c.correo?' · '+c.correo:'')); }); }
    y+=2;

    // ── Experiencia minera y plataformas ──
    ...
    if(exp.length||p.plataformas){ seccion('Experiencia minera AMSA');
      campo('Faenas: ',exp.join(' · ')); campo('Plataformas: ',p.plataformas); y+=2; }

    // ── Hotelería (si es hotel) ──
    if(esRubroHotel(p)){
      const hh=DB.hoteles[p._id]||{}; const cc=calcCamas(hh);
      seccion('Hotelería · habitaciones y servicios');
      campo('Habitaciones simples (1 cama): ', String(hh.simples||0)+(hh.simples_banio?'   ('+hh.simples_banio+' con baño privado)':''));
      campo('Habitaciones dobles (2 camas): ', String(hh.dobles||0)+(hh.dobles_banio?'   ('+hh.dobles_banio+' con baño privado)':''));
      campo('Total habitaciones: ', String((hh.simples||0)+(hh.dobles||0)));
      campo('Total camas: ', String(cc.total_camas));
      const tb=(hh.simples_banio||0)+(hh.dobles_banio||0);
      if(tb) campo('Habitaciones con baño privado: ', String(tb));
      if((hh.servicios||[]).length) campo('Servicios: ', (hh.servicios||[]).join(' · '));
      y+=2;
    }

    // ── Flota / Maquinaria ──
    if(p.flota&&p.flota.length){
      seccion('Maquinaria / Flota');
      const cols=[['Tipo',M,32],['Marca',M+34,28],['Modelo',M+64,26],['Año',M+92,16],['Cant.',M+110,16],['Capacidad',M+128,50]];
      doc.setFont('helvetica','bold');doc.setFontSize(7.5);doc.setTextColor(...GRAY);
      cols.forEach(c=>doc.text(c[0],c[1],y)); y+=1.5;
      doc.setDrawColor(...GOLD);doc.setLineWidth(.3);doc.line(M,y,W-M,y);y+=4;
      doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(...DARK);
      p.flota.forEach(f=>{ if(y>278){doc.addPage();y=20;}
        const vals=[f.tipo||'',f.marca||'',f.modelo||'',String(f.anio||''),String(f.cant||''),f.capacidad||''];
        cols.forEach((c,ci)=>{ const t=doc.splitTextToSize(String(vals[ci]),c[2]); doc.text(t[0]||'',c[1],y); });
        y+=5.5;
      });
      y+=2;
    }
    ...
    // ── Clasificación ──
    if(p.facturar||p.categoria_sii||p.agrupacion||p.servicio_am){ seccion('Clasificación');
      campo('Estado facturación: ',p.facturar); campo('Categoría SII: ',p.categoria_sii);
      campo('Agrupación: ',p.agrupacion); campo('Servicios AM: ',p.servicio_am); y+=2; }

    // ── Descripción ──
    if(p.descripcion&&p.descripcion.trim()){ seccion('Servicios y capacidades');
      doc.setFont('helvetica','normal');doc.setFontSize(8.5);doc.setTextColor(...DARK);
      const d=doc.splitTextToSize(p.descripcion.trim(),maxW); 
      d.forEach(l=>{ if(y>282){doc.addPage();y=20;} doc.text(l,M,y); y+=4.6; }); y+=2; }

    // ── Notas y comentarios ──
    if(p.notas_ficha&&p.notas_ficha.trim()){ seccion('Notas y comentarios');
      doc.setFont('helvetica','normal');doc.setFontSize(8.5);doc.setTextColor(...DARK);
      const n=doc.splitTextToSize(p.notas_ficha.trim(),maxW);
      n.forEach(l=>{ if(y>282){doc.addPage();y=20;} doc.text(l,M,y); y+=4.6; }); y+=2; }

    // ── Fotos ──
    if(p.fotos&&p.fotos.length){
      const fotosB64=await Promise.all(p.fotos.slice(0,3).map(u=>_urlToBase64(u).catch(()=>null)));
      const validas=fotosB64.filter(Boolean);
      if(validas.length){
        if(y>240){doc.addPage();y=20;}
        seccion('Fotografías');
    ...
    for(let i=1;i<=pages;i++){ doc.setPage(i);
      doc.setDrawColor(...GOLD); doc.setLineWidth(.5); doc.line(M,288,W-M,288);
      doc.setFont('helvetica','normal');doc.setFontSize(6.5);doc.setTextColor(...GRAY);
      doc.text('Antofagasta Minerals · Proveedores Comunitarios',M,292);
...
function toggleMnavCargar(){
  const b=document.getElementById('mnavCargarBody'), t=document.getElementById('mnavTri');
  if(!b)return; const open=b.style.display!=='none';
  b.style.display=open?'none':'block'; if(t)t.textContent=open?'▼':'▲';
}
...
// ═══ Subir minuta manualmente (fallback si el guardado automático falla) ═══
let _smPid=null;
function abrirSubirMinutaManual(pid){
  _smPid=pid;
  document.getElementById('smTitulo').value='';
  document.getElementById('smFecha').value=new Date().toISOString().slice(0,10);
  document.getElementById('smResponsable').value=miNombre()||'';
  document.getElementById('smArchivo').value='';
  document.getElementById('modalSubirMinuta').style.display='flex';
}
function cerrarSubirMinutaManual(){ document.getElementById('modalSubirMinuta').style.display='none'; _smPid=null; }
async function confirmarSubirMinutaManual(){
  const pid=_smPid; if(!pid) return;
  const titulo=document.getElementById('smTitulo').value.trim();
  const fecha=document.getElementById('smFecha').value;
  const responsable=document.getElementById('smResponsable').value.trim();
  const file=document.getElementById('smArchivo').files[0];
  if(!titulo||!fecha){ showToast('Completa título y fecha','err'); return; }
  if(!file){ showToast('Selecciona el archivo PDF de la minuta','err'); return; }
  if(file.type!=='application/pdf' && !file.name.toLowerCase().endsWith('.pdf')){ showToast('El archivo debe ser un PDF','err'); return; }
  const btn=document.getElementById('smBtnGuardar'); btn.disabled=true; btn.textContent='Subiendo...';
  try{
    const p=PROVEEDORES.find(x=>x._id===pid); if(!p) throw new Error('Proveedor no encontrado');
    const provId=p._proveedorId||pid;
    // Asegurar que el proveedor exista en Supabase antes de registrar
    const {data:chk,error:eChk}=await SUPA.client.from('proveedores').select('proveedor_id').eq('proveedor_id',provId).maybeSingle();
    if(eChk) throw new Error('Verificación proveedor: '+eChk.message);
    if(!chk){
      if(typeof gSyncPush==='function') await gSyncPush(pid);
      const {data:chk2}=await SUPA.client.from('proveedores').select('proveedor_id').eq('proveedor_id',provId).maybeSingle();
      if(!chk2) throw new Error('El proveedor aún no está guardado en la nube. Guárdalo primero (Editar → Guardar cambios).');
    }
    const url=await subirArchivo(file,'minutas');
    if(!url) throw new Error('No se pudo subir el archivo');
    const vid='manual_'+Date.now().toString(36);
    const {error:eVis}=await SUPA.client.from('visitas').upsert({
      visita_id:vid, proveedor_id:provId, fecha, titulo, responsable_nombre:responsable||'',
      responsable_email:'', faena:'', comuna:'', estado:'Finalizada', minuta_pdf_url:url,
      estado_registro:'Activo', origen_plataforma:'principal',
      created_by:miNombre(), updated_by:miNombre(), updated_at:new Date().toISOString()
    },{onConflict:'visita_id'});
    if(eVis) throw new Error('Visita: '+eVis.message);
    await registrarLog('visita',vid,'crear','Minuta subida manualmente para '+dispName(p));
    cerrarSubirMinutaManual();
    if(typeof renderVisitasV3==='function') renderVisitasV3(pid);
    showToast('✅ Minuta registrada correctamente','success');
  }catch(e){
    console.error('[MINUTA MANUAL] Error:',e);
    showToast('⚠ No se pudo registrar: '+e.message,'err');
  }finally{
